"""Refactor-mutation corpus harness (provledger.testing, phase 6). Pure and
DB-independent: the unit under test is a Matcher over NodeObs lists; extraction
is injected. The analyzer adapters live with the analyzer
(project-state-graph/scripts/tests/corpus/harness.py)."""
...
def default_corpus() -> Path:
    """The corpus that ships with the package: testing/corpus/cases."""
    return Path(__file__).resolve().parent / "corpus" / "cases"


def iter_cases(root: Path | None = None) -> list[Case]:
