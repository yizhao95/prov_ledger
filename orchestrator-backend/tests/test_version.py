"""0.3.0 release prep (phase 8 Task 5): one version string, everywhere it is read."""
import json
import re
...
def test_every_version_string_agrees():
    ...
    assert re.search(r'^version = "0\.3\.0"$', pyproject, re.M)
    store = (REPO / "skills" / "project-state-graph" / "scripts" / "analyzer" / "store.py").read_text()
    assert re.search(r'^TOOL_VERSION = "0\.3\.0"$', store, re.M)
    plugin = json.loads((REPO / ".claude-plugin" / "plugin.json").read_text())
    assert plugin["version"] == "0.3.0"
    market = json.loads((REPO / ".claude-plugin" / "marketplace.json").read_text())
    assert [p.get("version") for p in market["plugins"]] == ["0.3.0"]
