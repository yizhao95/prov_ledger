...
| [`docs/decision-provenance.md`](docs/decision-provenance.md) | why a node changed and where the why came from: the user's words verbatim (hook / `provledger note`), four tiers decided by structure (stated / asserted / derived / unstated), the computed 来源等级, the reason rules R1–R6, the non-rewriting migration of the old rows, the honest boundaries |
