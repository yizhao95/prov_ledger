...
| FL-088 | dp-phase2e | CLOSED（DP 3 期）| 证据卡的链头没有锚在数据库之外：`verify_chain` 只能证明"这份文件内部自洽"，一个能改行的人同样能重算整条链。卡片现在诚实地印 `not anchored`；真正的解法是把三条链的链头定期写进 `git notes`（或外部时间戳服务），卡片引用那个锚点。**3 期做了 git notes 这一半**：plan 关闭时 `api._anchor_close` 追加锚点，`provledger verify --against-notes` 比对，卡片印 `git note <sha> @ <commit>`。外部时间戳服务那一半见 FL-097 | 已关 |
...
| FL-097 | dp-phase3 | DEFERRED | 锚点只到本地仓库为止：能改账本又能 force-push `refs/notes/provledger` 的人可以同时改掉两边。需要一条「远端锚点」路径——推送后向远端查回 note 并比对（`git fetch origin refs/notes/provledger:refs/notes/provledger-remote` + 双向比对），或接一个外部时间戳服务，让锚点的信任不落在同一台机器上。`verify --against-notes` 现在不会区分「本地 note」与「远端确认过的 note」，这一点文档里说了，代码里还没有 | DP 4 期 |
| FL-098 | dp-phase3 | DEFERRED | bundle 是全量导出：每次 `export --out` 重写整个目录。真实用法是「上次给过他们了，这次只给增量」，需要 `--since <export_log.id \| 时间>` 产出增量 bundle，并在 manifest 里写明基线是哪一次导出（`export_log` 已经记了链头，够做基线），同时保证白名单与扫描在增量路径上一模一样 | DP 4 期 |
| FL-099 | dp-phase3 | DEFERRED | 收到 bundle 的人现在只能读 `manifest.json`，没有工具可以**独立核对**：需要一个不依赖本仓库的小核对器（单文件 Python，零依赖），输入 bundle + 一个 `git notes` 导出，重算每个文件的 sha256、比对 manifest 的链头与锚点，并打印「这份 bundle 与那个 commit 一致 / 不一致」。核对者不应该需要安装我们的包才能不信任我们 | DP 4 期 |
| FL-100 | dp-phase3 | DEFERRED | 导出扫描对短于 12 字符的 personal 文本只计数不匹配（`export.SCAN_MIN_LEN`）：一条 personal 原话是「开工」时，包含匹配会把每一次导出都拦下来，所以阈值是真实的取舍，但它也意味着**短的 personal 文本没有被扫描证明过**。应改成按词/句边界匹配（而不是裸 `in`），这样短文本也能扫而不误伤 | DP 4 期 |
