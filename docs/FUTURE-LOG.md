...
| FL-023 | phase3.5 | DEFERRED | dogfood：update-psg `stale_references` 闸门只看图不看 diff——同一 diff 里删掉的函数连同它唯一的调用点一起删除时仍报 stale（`_ensure_impact_column`），review 被打成 FAILED；`review_diff.report` 应像 `signature_contract` 一样把调用方文件在 changed_files 里的情形降为 warning | 阶段 4 前 |
| FL-024 | phase3.5 | DEFERRED | dogfood：`finish-plan.sh` 的"强制完成兜底"在 `review_and_complete` 返回 `ready=False` 时直接 `complete_plan`，包括 plan 刚进入 NEEDS_REVIEW 的情形（FL-022 重开 `p3-t3` 后 status=COMPLETED 而 review_state=awaiting_agent）；兜底必须排除 `needs_agent_review` | 阶段 4 前 |
