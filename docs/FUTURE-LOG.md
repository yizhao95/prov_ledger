...
| FL-021 | phase3 | DEFERRED | dogfood：`publish_plan.py` / `_apply_op._open_db` 只在 `Plans` 表缺失时才跑迁移，新迁移（014/015）对已有的 `~/skill-workspace/orchestrator.db` 静默不生效，阶段 3 靠手动 `db.run_migrations`；`schema_version.migration_file` 已能幂等跳过，应在每次 open 时直接跑 | 阶段 4 前 |
| FL-022 | phase3 | DEFERRED | dogfood：FL-019 修复只覆盖"REVIEW.1 失败后子树恢复"；plan 因**普通步骤未恢复**而直接 FAILED（review 步骤 FAILED 且无子步骤，如 `p3-t3`）时，事后在该步骤下补的恢复子步骤不会让 `review_and_complete` 重新评估——需要"review 无子且 FAILED、所有顶层失败步骤已恢复 → 重走判定"的路径 | 阶段 4 前 |
