#!/usr/bin/env bash
# headline-respond.sh — answer one finding of the plan headline (revise | proceed + rationale, cites become adopted).
...
set -uo pipefail
if [[ $# -ne 1 ]]; then
    echo "usage: headline-respond.sh <input.json|yaml|yml>" >&2
    exit 2
fi
...
exec "${PYBIN}" "${SCRIPT_DIR}/_apply_op.py" --op headline-respond "$1"
