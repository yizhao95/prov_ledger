#!/usr/bin/env bash
# start-step.sh — deterministic flow for start-step write op.
...
set -uo pipefail
if [[ $# -ne 1 ]]; then
    echo "usage: start-step.sh <input.json|yaml|yml>" >&2
    exit 2
fi
...
exec "${PYBIN}" "${SCRIPT_DIR}/_apply_op.py" --op start-step "$1"
