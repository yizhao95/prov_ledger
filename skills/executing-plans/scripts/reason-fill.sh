#!/usr/bin/env bash
# reason-fill.sh — deterministic flow for the reason-fill op: recording one reason per changed node (unstated -> NULL).
...
set -uo pipefail
if [[ $# -ne 1 ]]; then
    echo "usage: reason-fill.sh <input.json|yaml|yml>" >&2
    exit 2
fi
...
exec "${PYBIN}" "${SCRIPT_DIR}/_apply_op.py" --op reason-fill "$1"
