#!/usr/bin/env bash
# reason-slots.sh — deterministic flow for the reason-slots op: the closed-form reason checklist of a plan under review.
...
set -uo pipefail
if [[ $# -ne 1 ]]; then
    echo "usage: reason-slots.sh <input.json|yaml|yml>" >&2
    exit 2
fi
...
exec "${PYBIN}" "${SCRIPT_DIR}/_apply_op.py" --op reason-slots "$1"
