#!/usr/bin/env bash
# record-skill.sh — deterministic flow for record-skill write op.
...
set -uo pipefail
if [[ $# -ne 1 ]]; then
    echo "usage: record-skill.sh <input.json|yaml|yml>" >&2
    exit 2
fi
...
exec "${PYBIN}" "${SCRIPT_DIR}/_apply_op.py" --op record-skill "$1"
